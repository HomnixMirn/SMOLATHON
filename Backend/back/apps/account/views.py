...
def login(request: Request):
    if request.data.get("login") and request.data.get("password"):
        try: 
            user= authenticate(username = request.data.get("login"), password = request.data.get("password"))
            if user:
    ...
    return Response("Неправильное имя пользователя или пароль",status=status.HTTP_401_UNAUTHORIZED)
